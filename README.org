...
#+end_quote
The highlighted text is of importance.
...
#+end_src
After that run this in a admin cmd:
#+begin_src bat
  C:\Program Files\Looking Glass (host)\looking-glass-host.exe InstallService
#+end_src
Note: If the looking glass host running on vm doesn't run as admin, then any applications invoking admin requests or just running as admin won't be visible to us on screen. So make sure to run the above command in an admin cmd prompt window.
