...
2. Assuming you've emacs installed in your nixos system, run the command below
#+begin_src bash :tangle no
   emacs README.org --batch -f org-babel-tangle
#+end_src
   The above will generate all the .nix files.
3. Then run the nixos-rebuild command
#+begin_src bash :tangle no
   sudo nixos-rebuild switch --flake .#<lenovo/server>
#+end_src
4. For [[https://github.com/nix-community/home-manager][Home-Manager]] configuration, run the following while being in the repo directory.
#+begin_src bash :tangle no
